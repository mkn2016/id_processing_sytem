...
class AddStudentsForm(QDialog, RethinkDBOperations, AfricasTalkingSMS):
    def __init__(self):
    ...
    def save_file(self, filepath, student_doc):
        fh = open(filepath, 'rb')
        contents = fh.read()
        fh.close()
        doc_to_insert = {"id": student_doc["id"], "filename": filepath, "file": r.binary(contents)}

        self.insert_doc_to_table(BaseConfig.db, BaseConfig.files_table, doc_to_insert)
    ...
    def on_submit(self) -> NoReturn:
        ...
        if course_count == 0 or department_count == 0 or branch_count == 0:
            ...
            if not (male or female) or not (first_name and surname and id_passport and tel and email):
                ...
                id_result = self.generate_id(BaseConfig.db, BaseConfig.students_table)
                ...
                if id_result["result"] is None:
                    ...
                    qrcode_data = {"id": student_doc["id"], "status": student_doc["status"], "expiry_date": student_doc["expiry_date"], "credit_points": student_doc["credit_points"]}
                    to_added = "qrcode_images/" + str(student_doc["id"]) + ".png"
                    filepath = str(Path(__file__).parents[3].joinpath(to_added))
                    print(filepath)
                    encode_data_to_file(qrcode_data, filepath)
                    self.save_file(filepath, student_doc)

                    doc_insert_result = self.insert_doc_to_table(BaseConfig.db, BaseConfig.students_table, student_doc)
                    ...
                    if doc_insert_result["result"]:
                        if tel == "":
                            ...
                            q = Queue(maxsize=1)
                            m = Messenger(student_doc["id"], full_name, status)
                            Thread(target=self.send_message, args=(str(m), tel, q,)).start()
                            message_result = q.get()
                            insertion_message = f"Student saved successfully. {message_result['msg']}"
                            QMessageBox.information(self, "Success", insertion_message)
                            self.close()
                    else:
