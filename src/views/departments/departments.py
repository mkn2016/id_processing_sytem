...
class DepartmentsPage(QWidget, RethinkDBOperations):
    ...
    def setup_header(self) -> NoReturn:
        header_title = QLabel()
        header_title.setText("Departments")
        ...
        page_title = QLabel()
        page_title.setText("Dashboard/Departments")
    ...
    def setup_title(self) -> NoReturn:
        ...
        self.__title.setPlaceholderText("Enter Department Title")
    ...
    def load_departments_table_data(self) -> NoReturn:
        departments_docs = self.table_docs(BaseConfig.db, BaseConfig.departments_table)

        if isinstance(departments_docs, dict):
            QMessageBox.critical(self, "Critical", departments_docs["msg"])
        elif isinstance(departments_docs, list):
            if not departments_docs:
                pass
            else:
                self.__table.clearContents()
                self.__table.setRowCount(0)

                for row_no, row_data in enumerate(departments_docs):
                    ...
                    self.__table.setRowCount(len(departments_docs))
        ...
        if self.__edit_event:
            ...
            if not title:
                ...
                if title == doc["result"]["title"]:
                    ...
                    doc_update_result = self.update_doc(BaseConfig.db, BaseConfig.departments_table, department_doc,
                                                        department_id)
            ...
            if not title:
                ...
                print(title)
                id_result = self.generate_id(BaseConfig.db, BaseConfig.departments_table)
